...
for EVAL_SPECIES in human mouse rat rabbit opossum chicken macaque; do 
...
# Evaluation on left-out species
for EVAL_SPECIES in chicken macaque; do 
OUT_DIR=${DIR}/${RUN}/${PRED_DIR}/${EVAL_SPECIES}/
mkdir -p ${OUT_DIR}
python ${WORK_DIR}/scripts/evaluate_splice.py \
    --checkpoint "${CHECKPOINT_PATH}" \
