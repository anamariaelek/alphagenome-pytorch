...
For every (species, tissue):
  ...
  3. Assign each predicted trajectory to its nearest reference centroid
     (Euclidean, matching Ward's metric) → ``pred_cluster`` / ``pred_shape``.
...
Output (per species/tissue): ``<prefix>_prediction_clusters.parquet`` with
  ...
  true_cluster, true_shape, pred_cluster, pred_shape,
  same_cluster, same_shape, pred_centroid_dist, n_obs
...
def annotate(species, tissue, args):
    ...
    # 3. assign BOTH predicted and observed trajectories to the reference centroids
    ...
    # 4a. apples-to-apples: predicted vs observed, both via nearest-centroid
    out["same_cluster"] = out["pred_cluster"] == out["obs_cluster"]
    out["same_shape"] = out["pred_shape"] == out["obs_shape"]

    # 4b. vs the canonical reference Ward labels of the observed trajectory
    ref_meta = ref["meta"][["Chromosome", "Position", "Strand", "Cluster", "ClusterShape"]].copy()
    ref_meta["Chromosome"] = ref_meta["Chromosome"].astype(str)
    ref_meta = ref_meta.rename(columns={"Cluster": "ref_cluster", "ClusterShape": "ref_shape"})
    ...
    log.info("%s/%s: %s sites | pred-vs-obs: same_shape=%.3f same_cluster=%.3f | "
             ...
             out["same_shape"].mean(), out["same_cluster"].mean(),
             out["same_shape_ref"].mean(),
             (out["obs_shape"] == out["ref_shape"]).mean(), out_path)
    return out_path
