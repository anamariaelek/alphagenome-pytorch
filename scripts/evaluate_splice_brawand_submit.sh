...
# Finetuned model
RUN=lora_64_emb_traj_hqmrboc__
CHECKPOINT_PATH="${DIR}/${RUN}"
PRED_DIR=preds_${TIMESTAMP}

# Data configuration
DATA_CONFIG="${DIR}/data/brawand/data_config_${TIMESTAMP}.json"

# Evaluation on trained species
for EVAL_SPECIES in human gorilla chimpanzee bonobo macaque mouse; do 
OUT_DIR=${DIR}/${RUN}/${PRED_DIR}/${EVAL_SPECIES}_brawand/
mkdir -p ${OUT_DIR}
